...
Vagrant.configure('2') do |cfg|
  ...
  (0..number_of_worker_nodes).each do |i|
    ...
    cfg.vm.define vm_host_name do |config|
      ...
      if i == 0
        config.vm.provision :shell, name: :master, env: {'master_ip' => vm_private_ip}, inline: <<-SHELL.gsub(/^ +/, '')
          ...
          echo "$(echo "$kubeadm_init_message" | tail -1 | sed -e 's/^[[:space:]]*//') --ignore-preflight-errors=cri" > /vagrant/kubeadm-join.sh && chmod +x /vagrant/kubeadm-join.sh
